...
> TLS certs for HTTPS LAN access live in `config/tls/{cert,key}.pem` (generated by
> `scripts/tls-setup.sh`, gitignored like the rest of `config/`). See
> [HTTPS for LAN access](https-localhost.md).
