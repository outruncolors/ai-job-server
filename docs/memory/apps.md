...
### Prattletale
`character:<counterpart_character_id>` (facts that persist across **every** chat with that
character — the cross-conversation recall) and `session:<conversation_id>` (facts pinned to
one conversation). Use for who the user is, preferences, prior decisions, conversation
recaps. See the shipped wiring below.
...
scoped to `character:{{var.counterpart_id}}` + `session:{{var.session_id}}`, with the query
set to what the user just said (`{{var.mem_query}}`); `build_context` computes those
