...
| `app/apps/prattletale/voice.py` | SP6 voice + timing (additive, best-effort). Synthesizes spoken model items when `config.voice_enabled` **and** this node holds the `voice` capability: `dialogue`→counterpart's Hoodat `voice_preset_id`, `narration`/`narration_emotion`→app-level narrator voice; `action`/`system_error` + all user items never spoken. `synthesize_item(conversation, character, item)` is the **per-message** primitive (idempotent — reuses an existing wav) that backs the lazy client path; `synthesize_turn(...)` is the eager all-at-once helper built on it. `_synth_to_wav` is the single TTS boundary (test-stubbable). Degrades to text on any miss (no capability/preset/failure → `audio=None`, no media). `reveal_schedule(turn)` is the deterministic per-item timing record in the trace. |
