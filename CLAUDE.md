...
| `app/apps/prattletale/store.py` | File-per-conversation store (`CONVERSATIONS_DIR`, monkeypatchable); atomic writes; conversation CRUD + transcript ops `append_user_turn`/`append_model_turn`/`append_error_turn` (one `system_error` item)/`replace_turn` (in-place, same id/position — the Retry path)/`apply_audio` (set `audio` in place after synth)/`write_trace`. Append/replace ops **re-read** the transcript before writing (concurrent-write safe). Turn ids `t%04d`, item ids `<turn_id>-i%02d`. |
