...
| File | Purpose |
|------|---------|
| `app/apps/prattletale/models.py` | Pydantic `Conversation` (metadata + `DeviceUser` + `ConversationConfig` w/ `context_window_turns`/`voice_enabled`/`typing_timing_enabled`), `Transcript` (`turns` + monotonic `next_turn_seq`), `Turn`, `Item` (+ `audio` dict, `hidden_from_context`); `ItemType`/`Author`/`ItemStatus` enums. |
...
| `app/apps/prattletale/prompts.py` | Registers the Prompt Pal `(prattletale, turn)` entry — the tagged-line reply prompt (`[say]`/`[do]`/`[narration]`/`[feel]`, one bubble per line) + consumes `{{var.character}}`/`{{var.scenario}}`/`{{var.role_instructions}}`/`{{var.user_persona}}`/`{{var.transcript}}` — with a format-hygiene **guard** (every line tagged, leaked meta/OOC stripped, bubbles never merged). `parse_items(raw)`/`_strip_fences(raw)`: each `[tag] text` → one typed item (`say`→dialogue, `do`→action, `narration`→narration, `feel`→narration_emotion); untagged/unknown-tag line coalesces into the prior bubble; lone leading untagged line → dialogue; empty → `GenerationError`. |
| `app/apps/prattletale/generator.py` | `GenerationError`; `build_context(conversation, character, transcript)` (**pure**: recent-turn window, skips `hidden_from_context`/`system_error`, renders the Hoodat sheet + empty-persona placeholder); `build_turn_request` (`[turn, guard]` chain); `async run_model_turn(conversation_id, llm=None, *, replace_turn_id=None)` — load→build→`execute_chain_job`→`parse_items`→`append_model_turn`/`replace_turn`→best-effort voice→`write_trace`; **any** failure becomes a `system_error` turn (never raised to the router). |
| `app/apps/prattletale/voice.py` | SP6 voice + timing (additive, best-effort). After a text turn commits, synthesizes spoken model items when `config.voice_enabled` **and** this node holds the `voice` capability: `dialogue`→counterpart's Hoodat `voice_preset_id`, `narration`/`narration_emotion`→app-level narrator voice; `action`/`system_error` + all user items never spoken. `_synth_to_wav` is the single TTS boundary (test-stubbable). Degrades to text on any miss (no capability/preset/failure → `audio=None`, no media). `reveal_schedule(turn)` is the deterministic per-item timing record in the trace. |
...
| `app/apps/prattletale/router.py` | `/v1/apps/prattletale` — conversation CRUD (`GET`/`POST` [404 on missing counterpart]/`GET {id}`→`{conversation, transcript}`/`PATCH {id}` config toggles/`DELETE`), `GET {id}/media/{filename}` (basename-guarded), `GET`/`PUT /settings`, `POST {id}/turns` (append user turn → await `run_model_turn` → `{user_turn, model_turn}`), `POST {id}/turns/{turn_id}/retry` (latest model turn only; 409 otherwise; replaces in place). |
| `static/apps/prattletale/` | `index.html` + `prattletale.js` + `styles.css`: conversation list + **New conversation** form (pick counterpart, scenario, role instructions, persona); chat view of avatar-grouped bubble stacks; composer with **mode-cycling** (dialogue/action/narration) + multi-item drafting; client-side **typing indicator** during the POST; **reveal cadence** (`typingMs` mirrors `voice.py`) streaming bubbles + audio playback when timing is on; **error bubble + Retry** wired to the retry endpoint; voice/timing toggles + narrator-voice setting. Registered in `static/apps/apps.js`. |
