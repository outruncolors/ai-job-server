...
| File | Purpose |
|------|---------|
| `app/apps/hoodat/models.py` | `Character` (v1, nested `Appearance`/`Personality`/`Background`/`SpeakingStyle` blocks + top-level Identity fields + server `avatar_path`) + `CharacterDraft` (all-Optional LLM target). `SpeakingStyle` carries `dialogue_examples: list[str]` (few-shot voice samples; **not** in `FIELD_SPECS` — list-aware, generated separately). `FIELD_SPECS` (section→field→{label,kind}) is the single source of truth for generatable scalar fields (drives prompt registration, patch-building, normalization). |
...
| `app/apps/hoodat/prompts.py` | Registers Prompt Pal entries: `IDEATE`/`ASSEMBLE` (create chain) + one `field.<section>.<field>` per generatable field + `dialogue.example` (list-aware, `{{var.character}}`+`{{var.examples}}`) + `avatar.image_prompt` (templated). `render_character_context(doc)` (appends a "Dialogue examples:" block when present) / `avatar_prompt_variables(doc)` helpers. |
| `app/apps/hoodat/generator.py` | `run_create(name, prompt)` (ideate→assemble→parse w/ ≤2 retries→persist) + `run_field(id, section, field)` (single-step, normalize per kind, persist) + `run_dialogue_example(id, examples)` (single-step, `_normalize_dialogue` keeps internal newlines, **does not persist** — frontend owns the list). Runs `execute_chain_job` **directly** (not the queue), mirroring blaboratory. |
...
| `app/apps/hoodat/router.py` | `/v1/apps/hoodat` — characters CRUD, `POST .../fields/{section}/{field}/generate` (→`{value, prompt_id, job_id}`), `POST .../dialogue-examples/generate` (body `{examples}`, →`{value, prompt_id, job_id}`; caller persists the full list via CRUD `PUT`), avatar generate/upload/serve, exports list/run. Text gen ungated; **avatar generate degrades gracefully** (503 when `image` not in `get_local_capabilities()` — does NOT gate the whole app). |
| `static/apps/hoodat/` | `index.html`+`hoodat.js` (card list + search + create→redirect); `profile.html`+`profile.js` (Discord-style profile, 6 tabs, every field + the avatar wrapped with `FieldControls` for ✨generate / ✏️edit-prompt; tabs use uniform `.hd-section` cards via `sectionCard()`; Speaking Style integrates voice presets + `POST /v1/jobs/voice` sample + a **dialogue examples** list (add/generate-next/regenerate-row/remove; frontend owns the list); Exports tab). Registered in `static/apps/apps.js`. |
