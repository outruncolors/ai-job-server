...
### Prattletale — iMessage-style roleplay chat (`app/apps/prattletale/`, `static/apps/prattletale/`)

A human ↔ Hoodat-character text roleplay. Each conversation is a **self-contained folder**
`config/prattletale/conversations/<id>/` (`conversation.json` + `transcript.json` + `traces/<turn>.json` +
`media/<item>.wav`). A **turn** is one side's atomic contribution; an **item** is one rendered bubble
(`dialogue`/`action`/`narration`/`narration_emotion`/`system_error`/`summary`). The model turn runs
the chain executor **directly** (not the `JobQueue`), mirroring Hoodat/Blaboratory. Phase 1 (text
loop + voice), Phase 2 (config & dev tools), and Phase 3 (a **plugin system** — registry + manifest +
generic action dispatch + per-conversation `enabled_plugins` + a `window.PtPlugins` frontend loader,
shipping **Summarizer** as the first plugin) are built; see `docs/apps/prattletale/`.

| File | Purpose |
|------|---------|
| `app/apps/prattletale/models.py` | Pydantic `Conversation` (metadata + `DeviceUser` + `ConversationConfig` w/ `context_window_turns`/`voice_enabled`/`typing_timing_enabled`/`variety_pass_enabled` [default on]/`enabled_plugins` [Phase 3]), `Transcript` (`turns` + monotonic `next_turn_seq`), `Turn`, `Item` (+ `audio` dict, `hidden_from_context`); `ItemType` (incl. `summary`)/`Author` (incl. `system`)/`ItemStatus` enums. |
| `app/apps/prattletale/store.py` | File-per-conversation store (`CONVERSATIONS_DIR`, monkeypatchable); atomic writes; conversation CRUD (`update_conversation` shallow-merges, so the router deep-merges `config` itself) + transcript ops `append_user_turn`/`append_model_turn`/`append_error_turn` (one `system_error` item)/`append_summary_turn` (a `system`-authored turn with one `summary` item — Phase 3)/`replace_turn` (in-place, same id/position — the Retry path)/`apply_audio` (set `audio` in place after synth)/`write_trace` + in-place item/turn edits `edit_item`/`set_item_hidden`/`delete_item` (drops the turn when its last item goes → returns `{"turn_deleted": id}`)/`delete_turn` (no renumber — ids stay stable) + trace reads `get_trace`/`list_traces`. Append/replace/edit ops **re-read** the transcript before writing (concurrent-write safe). Turn ids `t%04d`, item ids `<turn_id>-i%02d`. |
| `app/apps/prattletale/prompts.py` | Registers two Prompt Pal entries: `(prattletale, turn)` — a detailed behavioral reply prompt (answer what was actually said, **shape per the message-style wildcard**, vary structure vs. your own recent messages, **no emoji/markdown**, mostly dialogue) emitting tagged lines (`[say]`/`[do]`/`[narration]`/`[feel]`), consuming `{{var.character}}`/`{{var.scenario}}`/`{{var.role_instructions}}`/`{{var.user_persona}}`/`{{var.transcript}}` plus a `%%Prattletale Message Style%%` wildcard token (resolved per-turn by the generator), with a format-hygiene **guard** (every line tagged, emoji/markdown/OOC stripped, lines never merged); and `(prattletale, variety)` — the anti-monotony rewrite pass that reworks wording but **preserves the chosen message shape** (line count/kinds). `parse_items(raw)`/`_strip_fences(raw)`/`_clean_text` (deterministic emoji scrub via `_EMOJI_RE`): each `[tag] text` → one typed item (`say`→dialogue, `do`→action, `narration`→narration, `feel`→narration_emotion); untagged/unknown-tag line coalesces into the prior bubble; lone leading untagged line → dialogue; emoji stripped + emptied items dropped; empty → `GenerationError`. |
| `app/apps/prattletale/seed.py` | `seed_message_style_wildcard()` — seed-if-absent of the `Prattletale Message Style` wildcard (the turn-shape distribution: 40% single message / 30% action+message / 20% mix-with-narration / 10% burst, as weighted natural-language entries). Called at lifespan startup (`app/main.py`); never clobbers a user-edited wildcard. The distribution is tunable in the Wildcards UI. |
| `app/apps/prattletale/generator.py` | `GenerationError`; `build_context(conversation, character, transcript)` (**pure**: recent-turn window, skips `hidden_from_context`/`system_error`, renders the Hoodat sheet + empty-persona placeholder; a `system`/`summary` turn renders as `[Summary so far] …` and is never dropped); `build_turn_request(context_vars, llm, *, variety=True)` — the `turn` → (`variety`) → `guard` chain (the optional **variety** pass is a middle `llm` step over `{{previous}}` that rewrites a draft repeating recent structure; gated by `config.variety_pass_enabled`, skipped when off/empty). The turn/variety prompts are run through `wildcards.resolve_wildcards` (server-side `%%name%%` expansion — fresh weighted pick per turn — since these prompts never hit the frontend resolver). `async run_model_turn(conversation_id, llm=None, *, replace_turn_id=None, synthesize=True)` — load→build→`execute_chain_job`→`parse_items`→`append_model_turn`/`replace_turn`→(eager voice when `synthesize`)→`write_trace`; **any** failure becomes a `system_error` turn (never raised to the router). The router passes `synthesize=False` (audio is produced per-message by the client instead). The success-path trace is **enriched** with an ordered `steps` list (`_collect_steps` pairs each request step with the executor's `steps/NNN_<id>/{prompt,output}.txt` → `{number, id, name, prompt, output}`, best-effort `output: null` on a read miss) so the trace is self-describing for the dev-tools node-graph. |
| `app/apps/prattletale/voice.py` | SP6 voice + timing (additive, best-effort). Synthesizes spoken model items when `config.voice_enabled` **and** this node holds the `voice` capability: `dialogue`→counterpart's Hoodat `voice_preset_id`, **every other non-error model item** (`action`/`narration`/`narration_emotion`)→app-level narrator voice; `system_error` + all user items never spoken. `synthesize_item(conversation, character, item)` is the **per-message** primitive (idempotent — reuses an existing wav) that backs the lazy client path; `synthesize_turn(...)` is the eager all-at-once helper built on it. `_synth_to_wav` is the single TTS boundary (test-stubbable). Degrades to text on any miss (no capability/preset/failure → `audio=None`, no media). `reveal_schedule(turn)` is the deterministic per-item timing record in the trace. |
| `app/apps/prattletale/settings_store.py` | App-level settings (`SETTINGS_PATH`, monkeypatchable) — currently the `narrator_voice_preset_id`. |
| `app/apps/prattletale/router.py` | `/v1/apps/prattletale` — conversation CRUD (`GET`/`POST` [404 on missing counterpart]/`GET {id}`→`{conversation, transcript}`/`PATCH {id}` broadened to edit metadata (`title`/`scenario`/`role_instructions`/`device_user`) **and** behaviour `config` — nested `config` patch or flat config keys hoisted in, deep-merged so unset keys survive, `context_window_turns` ≥ 1 (422)/`DELETE`), `GET {id}/media/{filename}` (basename-guarded), `GET`/`PUT /settings`, `POST {id}/turns` (append user turn → await `run_model_turn(synthesize=False)` → `{user_turn, model_turn}` — text returns immediately, audio is lazy), `POST {id}/turns/{turn_id}/retry` (latest model turn only; 409 otherwise; replaces in place), `POST {id}/turns/{turn_id}/items/{item_id}/audio` (per-message synth: `{audio: {path,duration_ms,voice_preset_id}|null}`, idempotent, persists via `apply_audio`), per-message edits `PATCH {id}/turns/{turn_id}/items/{item_id}` (`{text?, hidden_from_context?}` → updated turn)/`DELETE …/items/{item_id}` (updated turn or `{turn_deleted}`)/`DELETE {id}/turns/{turn_id}` (204), `GET {id}/turns/{turn_id}/trace` (the enriched debug trace; 404 absent), and **plugins** (Phase 3): `GET /plugins` (manifest list) + `POST {id}/plugins/{plugin_id}/actions/{action}` (generic dispatch — 404 unknown plugin/action, 409 when not in `config.enabled_plugins`, `ValueError`→422, else `await action.run(...)`). |
| `app/apps/prattletale/plugins/{base,registry}.py` | **Plugin system** (Phase 3). `base.py`: the `Plugin` dataclass (`id`/`name`/`description`/`version`/`frontend`/`actions`/`default_enabled`/`seed_prompts`) + `manifest()` (JSON-safe subset, action **names** only). `registry.py`: `PLUGIN_REGISTRY`, `register`/`get_plugin`/`list_plugins`, and `seed_plugins()` — imports each `_PLUGIN_MODULES` package (so its `register()` runs) and calls each plugin's `seed_prompts`. Wired into lifespan beside `seed_registered()`. |
| `app/apps/prattletale/plugins/summarizer/` | The **Summarizer** plugin (default-on). `plugin.py` registers `Plugin(id="summarizer", actions={"summarize": run_summarize}, frontend=[…])`; `run_summarize` validates `{mode: keep\|purge, detail, focus}` (bad→`ValueError`), snapshots the covered item ids, runs the engine, `append_summary_turn`, and on **purge** `set_item_hidden(True)` on each covered id → `{summary_turn, hidden_item_ids, mode}` (failure raises — never a chat `system_error` turn). `summarize.py`: `async summarize_history(...)` — hierarchical map-reduce over `execute_chain_job` (covered turns → chunk → map → reduce until one). `prompts.py`: editable Prompt Pal entries `summarize.map`/`summarize.reduce`/`summarize.level.{brief,standard,detailed}`. |
| `static/apps/prattletale/` | `index.html` + `prattletale.js` + `styles.css`: conversation list + **New conversation** form (pick counterpart, scenario, role instructions, persona); chat view of avatar-grouped bubble stacks; composer with **mode-cycling** (dialogue/action/narration) + multi-item drafting; the user's turn renders **optimistically on send** (before the synchronous POST returns) then a **typing indicator**; **reveal cadence** streams model messages one at a time: a **background producer** synthesizes the turn's clips in order, one at a time (`ensureAudio` dedupes via a shared in-flight promise, so the producer + reveal loop never double-fire a synth — at most one TTS job at once), while the reveal loop shows each message's "..." then **awaits that message's clip** (usually already produced → brief dots, `_MIN_TYPING_MS` floor; else dots stay until it's ready) and reveals the bubble with audio playback + a left→right **progress bar** (`runProgress`) across the read-aloud duration (clip length, else `typingMs` when no audio). Playback starts as soon as clip 0 is ready — never waits on the whole turn. The 🔊 button lazily synthesizes on click. **error bubble + Retry**; voice/timing/**variety** toggles (`variety` defaults on via `CONFIG_DEFAULT_ON`, sent as nested `{config:{…}}`) + narrator-voice setting. **Per-conversation config dialog** (⚙ in the chat header): title/scenario/role/display-name/persona + context-window + voice/timing/variety, saved via the broadened `PATCH`. **Per-message controls** via `FieldControls` (`/js/field-controls.js`, loaded in `index.html`): each bubble hover-exposes ✏️ edit (inline)/🚫 hide·👁 show/🗑 delete; the turn avatar exposes 🔍 trace (model turns) + 🗑 delete-turn; hidden bubbles render muted/struck-through; ops update `_current.transcript` and re-render the turn in place. **Trace viewer + node-graph** (`pt-trace-dialog`): `GET …/trace` → context bundle, raw/parsed output, reveal schedule, errors + a CSS `Turn → (Variety) → Guard` node-graph from the trace's `steps`, each node opening its prompt/output and deep-linking to its Prompt Pal entry (guard → the `turn` entry). **Plugins** (Phase 3): `plugins.js` defines `window.PtPlugins` (register + `composerModes`/`bubbleRenderer`/`panel` accessors); `prattletale.js` GETs `/plugins`, injects each enabled plugin's `frontend` assets on chat load, merges plugin composer modes onto Say/Do/Narrate, consults plugin bubble renderers in `bubbleHtml`, opens a plugin mode's slide-up options panel (CSS bounce; the composer stays put — its input is reused as the mode's free-text field via `placeholder`, and the **Send button becomes "Go"** running the plugin's `submitPanel`, independent of the staged draft; `pluginCtx` exposes `invokeAction`/`onResult`/`appendTurn`/`markHidden`/`close`/`panelEl`/`primaryValue`), and the config dialog's **Plugins** section toggles `config.enabled_plugins`. `plugins/summarizer/{summarizer.js,summarizer.css}` register the 📋 Summarize panel (Keep/Purge · detail in the panel; focus in the composer input; Go to run) + the centered `summary` card. Registered in `static/apps/apps.js`. |
